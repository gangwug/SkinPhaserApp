...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      fluidRow(
        column(6,
               fileInput('file1', label=h3('Choose data file'),
                         accept=c('text/csv', 
                                  'text/comma-separated-values,text/plain', '.csv')) ),
        column(6,
               radioButtons('fstyle', label=h3('File style'),
                            choices=c("csv"='csv', "txt"='txt'),
                            selected='csv', inline=FALSE) )
      ),
     fluidRow(
       column(12,
              radioButtons('markerGenesReminder', label=h3('How many marker genes were measured in epidermal samples?'),
     ...
     fluidRow(
       column(12,
              textInput("normGenes", label=h3("Internal control genes used for normalization"), value="GPKOW, BMS1, ANKFY1") )
     ),
     br(),
     br(),
     fluidRow(
       column(3,
              actionButton("update", label=h3("Run")) ),
       column(3,
              downloadButton('downloadData', label=h3('Download') ) )
     )
    ),
